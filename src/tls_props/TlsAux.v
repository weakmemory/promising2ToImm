...
Context
  ...
  Proof using.
    seq_rewrite (dom_rel_helper dom_sc_covered).
    basic_solver.
  Qed.
  ...
  Proof using.
    (* TODO: follows from IORDCOH : iord_coherent *)
  Admitted.
  ...
  Proof using RELCOV. etransitivity; [|apply dom_F_sb_I_in_C]; mode_solver 21. Qed.

  Lemma dom_F_Acq_sb_I_in_C :  dom_rel (⦗F ∩₁ Acq⦘ ⨾  sb ⨾ ⦗issued T⦘) ⊆₁ covered T.
  Proof using RELCOV. etransitivity; [|apply dom_F_sb_I_in_C]; mode_solver 12. Qed.
  
  Lemma dom_release_issued :
    dom_rel (release ⨾ ⦗ issued T ⦘) ⊆₁ covered T.
  Proof using WF IMMCON RELCOV.
